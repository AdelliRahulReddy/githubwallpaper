...
class WallpaperService {
  ...
  // 🔌 Platform channel (matches MainActivity.kt)
  ...
  static Future<bool> refreshAndSetWallpaper({String target = 'both'}) async {
    ...
    try {
      ...
      // 🖼️ Set wallpaper via platform channel (with timeout)
      final result = await _channel
          .invokeMethod<bool>('setWallpaper', {
            'path': file.path,
            'target': target,
          })
          .timeout(
            ...
            onTimeout: () =>
                throw TimeoutException('Platform channel timed out'),
          );

      if (result != true) {
        throw WallpaperException('Platform channel returned false');
      }
      ...
      debugPrint('WallpaperService platform error: ${e.message}');
      return false;
    } catch (e, stackTrace) {
  ...
  static Future<File> _generateWallpaper(CachedContributionData data) async {
    ...
    try {
      ...
      if (dimensions != null &&
        ...
        debugPrint(
          'WallpaperService: Using system dimensions ${targetWidth}x$targetHeight',
        );
      }
    } catch (e) {
      debugPrint('WallpaperService: Failed to get dimensions, using default: $e');
    }
